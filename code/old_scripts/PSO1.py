...
class PSO_asynchron(PSO):
    ...
    def worker(self, input, output):
        ...
        for part in iter(input.get, 'STOP'):
    ...
    def run(self, verbose=True):
        ...
        c = 0
        while c <= self.stop_queuing and not self.hasConverged:
            self.master(task_queue, done_queue)
            c += 1
        ...
        if verbose:
            print("After {} function evaluations,\nFound minimum at {} with value {}.".format(c, 
                                                                                              self.global_best,
                                                                                              self.global_best_fitness))
